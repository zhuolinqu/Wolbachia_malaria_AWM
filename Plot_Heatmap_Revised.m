...
for j1 = 1:length(ci_vec)
    P.ci = ci_vec(j1);
    P.vw = -((-2*(1+P.ci)+((-1+P.ci)^2*P.mufw*P.phiU)/(P.mufu*P.phiW) + (P.mufu*P.phiW)/(P.mufw*P.phiU))/(4*P.ci));
    P.vu = 1-P.vw;
    ...
    [Sustar, Swstar] = EquilibriumState(r,P);
    %Sustar = P.Kf*(1-1/G0u);
    %Swstar = 0;
...
%}
            ...
            id3 = find(outputM(j2,j1,:)<1,1,'last');
            if ~isempty(id3)
                outsurf(j2,j1) = alpha_vec(id3);
            else 
                outsurf(j2,j1) = NaN;
            end
...
vwmat = repmat(vw_vec,21,1);
cimat = repmat(ci_vec',1,26);

figure
scatter3(vwmat,cimat,outsurf,'k*')
%}